package br.com.compasso.pet.handler;
...
public class HandlerException {
    ...
    protected ResponseEntity<MensagemErro> handlerOngJaCriada(ResponseStatusException exception) {
        if (exception.getStatus() == HttpStatus.OK){
            return ResponseEntity.status(HttpStatus.OK).body(new MensagemErro("Pet already exists."));
        }
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new MensagemErro("Pet not found."));
    }
